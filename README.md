...
如果目标就是登录页 / 登录功能（校验、空提交、错误提示、布局等）：

1. **不要勾选**「等待手动登录」
2. 直接把登录页 URL 作为目标开始测试
3. Agent 会把登录页当被测对象继续测，而不会停下来要你输入账号密码
4. 若某条用例需要“登录成功”但未提供测试账号，会标为阻塞，同时继续完成不依赖真实账号的检查

只有要测登录后的业务页时，才使用下面的登录态方案。
