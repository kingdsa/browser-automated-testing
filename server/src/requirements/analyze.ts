...
- 逐模块分析，但避免重复测试点；结论必须足以直接生成不遗漏功能点的思维导图。
...
export async function streamAnalyzeRequirementDocument(input: {
  ...
  try {
    const reasoningSystemPrompt = `你是一名资深黑盒测试分析师。${ANALYSIS_OUTPUT_CONSTRAINT}`
...
export async function streamGenerateMindMap(input: {
  ...
  try {
    const recordSystemPrompt = MINDMAP_RECORD_OUTPUT_CONSTRAINT.trim()
    const client = createClient(llm)
