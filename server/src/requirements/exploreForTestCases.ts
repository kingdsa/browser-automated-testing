...
function buildExplorationSystemPrompt(targetUrl: string | undefined, skillMarkdown: string) {
  return [
    ...
    '后续会把探索笔记直接改写成测试步骤，因此路径必须足够精确。',
    '不要输出测试用例 JSON，不要输出 Markdown 测试报告。',
    '',
    '## 硬性约束',
    '1. 只能描述真实观察到的页面内容、入口、文案、交互与反馈；禁止编造不存在的按钮/菜单/路由。',
    '2. 功能点只是覆盖范围提示：优先探索与功能点相关的入口；找不到对应入口时明确写“未找到”。',
    '3. 步骤描述必须是原子浏览器操作，使用业务语言与真实可见文案：',
    '   - 好：选择左侧菜单「用户管理」',
    '   - 正确：在列表中勾选一条用户',
    '   - 正确：点击右上角「删除」按钮',
    '   - 正确：在确认弹窗点击「确定」',
    '   - 错误：进入用户相关入口 / 完成删除主流程',
    '4. 不要输出 CSS selector / XPath / Playwright 代码。',
    '5. 深度探索：不要只看首屏。应继续点击主导航、关键按钮、表单入口，覆盖多步路径。',
    '6. 破坏性写操作策略：',
    '   - 默认不要真正执行支付、删除确认、提交订单等会改数据的最终动作。',
    '   - 但必须尽量探索到对应按钮/弹窗，并记录完整可执行路径与可见文案。',
    '   - 可写“已看到确认弹窗「确定/取消」，未真正点击确定”。',
    '7. 登录策略：',
    ...
    '4. 记录菜单名、列表操作、右上角/行内按钮、弹窗按钮、结果反馈的真实文案',
    '5. 必要时 get_console_logs / get_network_logs / take_screenshot',
    '6. 信息足够后停止工具调用，输出完整探索笔记',
    '',
    '## 最终输出格式（纯文本）',
    '### 页面概况',
    '- 实际 URL / 标题 / 是否登录',
    '### 关键可交互入口',
    '- 菜单、Tab、按钮、链接、表单字段的可见文案与大致位置（左侧/顶部/右上角/行内/弹窗）',
    '### 已验证路径',
    '每条路径必须拆成有序原子步骤，禁止一句话概括：',
    '- 路径 1：删除用户',
    '  1. 选择左侧菜单「用户管理」',
    '  2. 在用户列表勾选一条记录',
    '  3. 点击右上角「删除」',
    '  4. 确认弹窗出现「确定」「取消」',
    '  5. 页面反馈：...（若未真正提交，写明只观察到弹窗）',
    '- 路径 2：...',
    '### 与功能点对应关系',
    '- 功能点 A：找到入口 / 未找到 / 阻塞原因；对应哪条已验证路径',
    '### 校验与反馈',
    '- 成功/失败提示、空态、加载态、确认弹窗文案等',
    '### 风险与阻塞',
    '- 登录、权限、环境问题',
    '',
    targetUrl ? `## 本次目标 URL\n${targetUrl}` : '## 本次目标 URL\n由当前已附着标签或用户消息指定',
...
function buildExplorationUserPrompt(input: {
  ...
  return [
    ...
    '',
    '重点：',
    '1. “已验证路径”必须写成可直接变成测试步骤的原子操作（选择菜单 / 选中数据 / 点击按钮 / 确认弹窗 / 查看结果）。',
    '2. 禁止写“相关入口”“完成主流程”这类空泛描述。',
    '3. 最终回复只写探索笔记，不要写测试用例。',
  ].join('\n')
}
...
export async function explorePageForTestCases(input: {
  ...
  try {
    ...
    if (!finalNotes) {
      ...
      history.push({
        role: 'user',
        content: '请停止继续操作，仅根据目前已观察结果输出完整「页面探索笔记」。已验证路径必须拆成原子步骤（菜单/对象/按钮/确认/结果），禁止“相关入口”等空泛描述。不要再调用工具。',
      })
